...
def main(
    ...
    root_files = [
        f
        for f in Path(amass_root_dir).glob("*.[np][pk][lz]")
    ...
    if not force_remake:
        root_files_to_process = [
            ...
            if not (
                amass_root_dir / f"{f.name.replace('.npz', '.npy').replace('.pkl', '.npy').replace('-', '_').replace(' ', '_').replace('(', '_').replace(')', '_')}"
            ).exists()
        ]
    else:
    ...
    if root_files_to_process:
        ...
        data_dir = amass_root_dir
        output_dir = amass_root_dir / append_name
        ...
        for filename in tqdm(root_files_to_process):
            ...
            if force_retarget:
                ...
                motion_data = {
                    k: np.array(v) if k != 'fps' else int(v) for k, v in motion_data.items()
                }
