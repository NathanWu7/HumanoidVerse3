...
from tools.motion_lib.motion_utils.flags import flags
from enum import Enum
from tools.motion_lib.skeleton import SkeletonTree
...
class MotionLibBase():
    ...
    def get_motion_actions(self, motion_ids, motion_times):
        ...
        length_starts = self.length_starts.to(frame_idx0.device)
        f0l = frame_idx0 + length_starts[motion_ids]
        f1l = frame_idx1 + length_starts[motion_ids]
    ...
    def _calc_frame_blend(self, time, len, num_frames, dt):
        time = time.clone()
        len = len.to(time.device)
        device = time.device

        # 保证 num_frames 和 dt 都是 tensor 并且在同一 device
        if not torch.is_tensor(num_frames):
